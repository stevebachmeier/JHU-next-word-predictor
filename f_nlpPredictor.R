...
nlpPredictor <- function(input) {
  ...
  # input="What are we going to do at the end of " # Test 5-gram model
  ...
  search_tokens <- tail(input_tokens[[1]], 4)
  ...
  ngrams_matched_bo4$pred <- ngrams_matched_bo4$ngram
  ...
  predictions <- data.table(word=preds$pred, S_0=preds$S, S_bo1=preds_bo1$S, 
                            S_bo2=preds_bo2$S, S_bo3=preds_bo3$S, S_bo4=preds_bo4$S,
                            S=(preds$S + preds_bo1$S + preds_bo2$S + preds_bo3$S + preds_bo4$S))

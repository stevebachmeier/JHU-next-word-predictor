...
nlpPredictor <- function(input) {
  ...
  preds <- data.table(pred=all_1grams$ngram)
  preds_bo1 <- data.table(pred=all_1grams$ngram)
  preds_bo2 <- data.table(pred=all_1grams$ngram)
  preds_bo3 <- data.table(pred=all_1grams$ngram)
  
  # Initial search
  source("f_cleanWords.R", echo=F)
  ...
  if (nrow(ngrams_matched_bo1) != 0) {
    ...
    preds_bo1$S_bo1[is.na(preds_bo1$S_bo1)] <- 0
  } else {
    preds_bo1$S_bo1 <- 0
  }
  ...
  if (nrow(ngrams_matched_bo2) != 0) {
    ...
    preds_bo2$S_bo2[is.na(preds_bo2$S_bo2)] <- 0
  } else {
    preds_bo2$S_bo2 <- 0
  }
  ...
  if (nrow(ngrams_matched_bo3) != 0) {
    ...
    preds_bo3$S_bo3[is.na(preds_bo3$S_bo3)] <- 0
  } else {
    preds_bo3$S_bo3 <- 0
  }
  
  # Combine predictions
  predictions <- data.table(word=preds$pred, S0=preds$S0, S_bo1=preds_bo1$S_bo1, 
  ...
  cat(paste0("<font color=\"#000\" size=\"5\"><b>", predictions[1]$word, "</b></font>", "    (S=",round(predictions[1]$S, 2),")", "<br>",
             ...
             "  ", "<font color=\"#000\" size=\"4\">", predictions[3]$word, "</font>", "    (S=",round(predictions[3]$S, 2),")", "<br>"))
